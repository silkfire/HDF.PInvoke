...
public partial class H5ATest
{
    ...
    public void H5Aopen_by_nameTest1()
    {
        ...
        att = H5A.open_by_name(m_v2_test_file, dotNamePtr, aNamePtr);
        Assert.True(att >= 0);
        Assert.True(H5A.close(att) >= 0);
        ...
        att = H5A.open_by_name(m_v0_test_file, dotNamePtr, aNamePtr);
        Assert.True(att >= 0);
        Assert.True(H5A.close(att) >= 0);
    ...
    public void H5Aopen_by_nameTest2()
    {
        ...
        Assert.False(H5A.open_by_name(Utilities.RandomInvalidHandle(), dotNamePtr, emptyNamePtr) >= 0);
        Assert.False(H5A.open_by_name(H5AFixture.m_v2_class_file, dotNamePtr, emptyNamePtr) >= 0);
