...
public partial class H5ATest
{
    ...
    public void H5Aiterate_by_nameTest1()
    {
        ...
        Assert.True(H5A.iterate_by_name(m_v2_test_file, dotNamePtr, H5.index_t.NAME, H5.iter_order_t.NATIVE, ref n, Callback, new nint(-99)) >= 0);
        ...
        al.Clear();
        n = 0;
        Assert.True(H5A.iterate_by_name(m_v0_test_file, dotNamePtr, H5.index_t.NAME, H5.iter_order_t.NATIVE, ref n, Callback, new nint(-99)) >= 0);
        // we should have 3 elements in the array list [now obsolete]
        Assert.True(al.Count == 3);
        Assert.DoesNotContain(al.ToArray(), s => s == null || string.IsNullOrEmpty(s.ToString()));
        // let's be specific
