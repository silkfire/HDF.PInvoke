...
public partial class H5LTest
{
    ...
    public void H5LvisitTest1()
    {
        ...
        Assert.True(H5L.visit(m_v0_test_file, H5.index_t.NAME, H5.iter_order_t.NATIVE, cb, op_data) >= 0);
        // we should have 5 elements in the array list
        Assert.True(al.Count == 5);

        Assert.True(H5L.visit(m_v2_test_file, H5.index_t.NAME, H5.iter_order_t.NATIVE, cb, op_data) >= 0);
