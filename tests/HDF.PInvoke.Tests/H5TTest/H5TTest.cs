...
public sealed partial class H5TTest : IClassFixture<H5TFixture>, IDisposable
{
    ...
    public H5TTest()
    {
        ...
        Assert.True(H5T.INTEL_I8 > 0);
        Assert.True(H5T.INTEL_I8 == H5T.STD_I8LE);

        Assert.True(H5T.MIPS_B16 > 0);
        Assert.True(H5T.MIPS_B16 == H5T.STD_B16BE);

        Assert.True(H5T.ALPHA_F32 > 0);
        Assert.True(H5T.ALPHA_F32 == H5T.IEEE_F32LE);
    }
