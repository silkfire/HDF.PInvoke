...
public partial class H5STest
{
    ...
    public void H5Sget_select_boundsTest1()
    {
        ...
        Assert.True(H5S.get_select_bounds(space, start, stop) >= 0);
        for (int i = 0; i < dims.Length; ++i)
        {
            Assert.True(start[i] == 0);
            Assert.True(stop[i] == dims[i] - 1);
        }
    ...
    {
        Assert.False(H5S.get_select_bounds(Utilities.RandomInvalidHandle(), null, null) >= 0);
    }
}
