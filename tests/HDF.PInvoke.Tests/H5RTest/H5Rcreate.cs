...
public partial class H5RTest
{
    ...
    public void H5RcreateTest2()
    {
        ...
        ssize_t size = H5R.get_name(m_v2_test_file, H5R.type_t.OBJECT, hnd.AddrOfPinnedObject(), (byte[])null, ssize_t.Zero);
        Assert.True(size.ToInt32() == name.Length);

        byte[] buf = new byte[size.ToInt32() + 1];
        size = H5R.get_name(m_v2_test_file, H5R.type_t.OBJECT, hnd.AddrOfPinnedObject(), buf, new ssize_t(buf.Length));
        Assert.True(size.ToInt32() == name.Length);
