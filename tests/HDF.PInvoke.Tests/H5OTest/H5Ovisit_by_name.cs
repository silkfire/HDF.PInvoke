...
public partial class H5OTest
{
    ...
    public void H5Ovisit_by_nameTest1()
    {
        ...
        Assert.True(H5O.visit_by_name(m_v0_test_file, aStringPtr, H5.index_t.NAME, H5.iter_order_t.NATIVE, cb, op_data) >= 0);
        // we should have 4 elements in the array list
        Assert.True(al.Count == 4);

        Assert.True(H5O.visit_by_name(m_v2_test_file, aStringPtr, H5.index_t.NAME, H5.iter_order_t.NATIVE, cb, op_data) >= 0);
    ...
    public void H5Ovisit_by_nameTest2()
    {
        ...
        var firstUtf8StringPtr = Marshal.StringToCoTaskMemUTF8(path);
        ...
        Assert.True(H5O.visit_by_name(m_v0_test_file, firstUtf8StringPtr, H5.index_t.NAME, H5.iter_order_t.NATIVE, cb, op_data) >= 0);
        // we should have 5 elements in the array list
        Assert.True(al.Count == 5);

        Assert.True(H5O.visit_by_name(m_v2_test_file, firstUtf8StringPtr, H5.index_t.NAME, H5.iter_order_t.NATIVE, cb, op_data) >= 0);
