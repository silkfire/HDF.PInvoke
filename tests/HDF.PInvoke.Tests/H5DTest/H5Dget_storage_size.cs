...
public partial class H5DTest
{
    ...
    public void H5Dget_storage_sizeTest1()
    {
        ...
        hid_t dset = H5D.create(m_v0_test_file, dsetStringPtr, H5T.STD_I16LE, space, H5P.DEFAULT, dcpl);
        Assert.True(dset >= 0);
        Assert.True(H5D.get_storage_size(dset) == 4194304);
        Assert.True(H5D.close(dset) >= 0);

        dset = H5D.create(m_v2_test_file, dsetStringPtr, H5T.STD_I16LE, space, H5P.DEFAULT, dcpl);
        Assert.True(dset >= 0);
        Assert.True(H5D.get_storage_size(dset) == 4194304);
        Assert.True(H5D.close(dset) >= 0);
