...
public partial class H5DTest
{
    ...
    {
        var vlenStringPtr = Marshal.StringToHGlobalAnsi("dset");
        ...
        Assert.True(H5D.write(dset, vlen, H5S.ALL, H5S.ALL, H5P.DEFAULT, wdata_hndl.AddrOfPinnedObject()) >= 0);
        Assert.True(H5D.write(dset1, vlen, H5S.ALL, H5S.ALL, H5P.DEFAULT, wdata_hndl.AddrOfPinnedObject()) >= 0);
        wdata_hndl.Free();
        ...
        Assert.True(H5D.read(dset, vlen, H5S.ALL, H5S.ALL, H5P.DEFAULT, rdata_hndl.AddrOfPinnedObject()) >= 0);

        for (int i = 0; i < rdata.Length; ++i)
        {
            Assert.True(rdata[i].len.ToInt32() == i + 1);
        }

        Assert.True(H5D.vlen_reclaim(vlen, space, H5P.DEFAULT, rdata_hndl.AddrOfPinnedObject()) >= 0);

        Assert.True(H5D.read(dset1, vlen, H5S.ALL, H5S.ALL, H5P.DEFAULT, rdata_hndl.AddrOfPinnedObject()) >= 0);
