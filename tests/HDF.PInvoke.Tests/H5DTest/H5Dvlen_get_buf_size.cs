...
public partial class H5DTest
{
    ...
    public void H5Dvlen_get_buf_sizeTest1()
    {
        ...
        Assert.True(H5D.vlen_get_buf_size(dset, vlen, space, ref size) >= 0);
        Assert.True(size == 220); // (1 + 2 + ... + 10) x sizeof(int)

        Assert.True(H5D.vlen_get_buf_size(dset1, vlen, space, ref size) >= 0);
        Assert.True(size == 220); // (1 + 2 + ... + 10) x sizeof(int)
